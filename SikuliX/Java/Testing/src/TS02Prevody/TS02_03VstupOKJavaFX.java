	...
	public void TC02_03_01CeleCislo() {
		if (run) {
			try {
				...
				assertTrue("Očekáváno: 5, zjištěno něco jiného", s.find("png/java/vystupLabel.png").right(200).grow(0,
						10).exists("png/java/vystup5.png") != null);
		...
		if (run) {
			try {
				...
				assertTrue("Očekáváno: 5.1, zjištěno něco jiného", s.find("png/java/vystupLabel.png").right(200).grow
						(0, 10).exists("png/java/vystup5_1.png") != null);
			} catch (FindFailed | AssertionError e) {
				s.capture().save("errors", screenshotName());
				logger.error(e.getMessage());
				fail(e.getMessage());
